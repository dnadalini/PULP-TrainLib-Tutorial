...
parser.add_argument( '--transpose', type=int, default=0) # if ==1, the weigths are saved as transposed
...
temp_value = -1
if simple_kernel:
    initial_weights[0:out_size] = 1e-3
else:
    for i in range(out_size):
        for j in range(in_size):
            initial_weights[i][j] = temp_value
            temp_value = temp_value + 1e-3
...
if transp_wgt == 0:
    ...
    wgt_t = net.lin.weight.detach().clone()
    wgt_t = wgt_t.transpose(0, 1)
    f.write('PI_L2 fp16 L0_WEIGHTS_params[L0_WEIGHTS] = {'+dump.tensor_to_string(wgt_t)+'};\n')
else:
    print("Invalid TRANSPOSE_WEIGHTS parameter!!")
    exit()
...
for i in range(1):
    ...
    if transp_wgt == 0:
        ...
        wgt_g_t = parameter.grad.detach().clone()
        wgt_g_t = wgt_g_t.transpose(0, 1)
        f.write('PI_L2 fp16 L0_WEIGHT_GRAD [L0_WEIGHTS] = {'+dump.tensor_to_string(parameter.grad)+'};\n')
